...
export async function runPollingCycle({ apiBaseUrl, sendDiscord }) {
  const scanResponse = await hitApi(apiBaseUrl, "/api/scan-rss-feed");
  const ciks = [...new Set(getMatchedCiksFromScan(scanResponse))];

  let cikJsonCount = 0;
  for (const cik of ciks) {
    await hitApi(apiBaseUrl, `/api/cik-json?cik=${encodeURIComponent(cik)}`);
    cikJsonCount += 1;
  }
