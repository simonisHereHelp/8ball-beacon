...
function formatActualEps(row: FinnhubEarning): string {
  if (typeof row.epsActual === "number") {
    return `Actual: ${formatUsdPerShare(row.epsActual)}`;
  }

  if (row.date && row.hour) {
    const hourLabel = row.hour
      .replace("bmo", "Pre-Market")
      .replace("amc", "After Market Close")
      .replace("dmh", "During Market Hours");
    return `Actual: Available ${row.date}, ${hourLabel}`;
  }

  if (row.date) return `Actual: Available ${row.date}`;
...
function formatRows(rows: FinnhubEarning[]): string {
  return rows
    .map((row) => {
      ...
      const currentEps = typeof row.currentEps === "number" ? ` | current EPS (TTM): ${formatUsdPerShare(row.currentEps)}` : "";
      const estimated = typeof row.epsEstimate === "number"
        ? `Estimated EPS: ${formatUsdPerShare(row.epsEstimate)}`
        : "Estimated EPS: N/A";
      const actual = formatActualEps(row);
