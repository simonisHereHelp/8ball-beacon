...
- New application page: https://discord.com/developers/applications?new_application=true
...
DISCORD_FILINGS1_CHANNEL_ID=CHANNEL_ID_FOR_FILINGS1
...
# Optional debug tracing (OFF by default; uses DISCORD_WEBHOOK_URL)
DISCORD_BOT_DEBUG=false

# Optional for Python smart intent classification
OPENAI_API_KEY=sk-...
...
### Optional bot debug tracing

If you need to diagnose Discord auth/permission issues, enable temporary debug:

```bash
DISCORD_BOT_DEBUG=true
```

When enabled, the bot sends debug messages to `DISCORD_WEBHOOK_URL`, including:
- request path
- masked token check
- masked auth header snapshot
- Discord API non-2xx response summaries

Turn debug off after troubleshooting.
...
```bash
npm run bot:discord
# or run the Python conversational bot
npm run bot:discord:py
# or run the test-channel bot
npm run bot:test
```

`bot:test` workflow:
- On start: sends `hello world` to `#filings2`
- Monitors `#filings1`; if message equals `simon say`, it sends `simon say hi` to `#filings2`
...
### Python conversational bot option

If you prefer a smarter chat flow (confirm/run/result/next-step), run:

```bash
npm run bot:discord:py
```

This runs `scripts/discord-control-bot.py`, which keeps API routes unchanged and supports conversational intents for `scan rss`, `cik <CIK>`, `log`, and `help` with confirm/cancel prompts.

When `OPENAI_API_KEY` is set, the Python bot keeps rule-based matching first, then uses ChatGPT as a fallback intent classifier for natural language.
